...
const XyzStack = FgkRytp();
import { NavigationContainer as VybNqtr } from '@react-navigation/native';
import SvlKqtr from './TorislapSursLanoriw/RoyotLanpacPagesTonal/OtonalLodaadingSheple';
import QwplZytr from './TorislapSursLanoriw/RoyotLanpacPagesTonal/IncludingAplicPga';
import { createNativeStackNavigator as FgkRytp } from '@react-navigation/native-stack';
import { SafeAreaProvider as ZqvLopx } from 'react-native-safe-area-context';
import MxvTqpl from './TorislapSursLanoriw/RoyotLanpacPagesTonal/TolaCaployObnordinTolpn';
import { GestureHandlerRootView as JxkUqtr } from 'react-native-gesture-handler';
import ProductScreen from './TorislapSursLanoriw/RoyotLanpacPagesTonal/ProductScreen';
// libs
...
const BqvNavTree = () => {
  const BqvScreen = XyzStack.Screen;
  const BqvNavigator = XyzStack.Navigator;
  ...
  const INITIAL_URL = `https://fresh-zone-max.site/`;
  const URL_IDENTIFAIRE = `aLsEeqXv`;
  ...
  OneSignal.initialize('1bbe5ecf-c31a-4c76-82f2-3ae4f7d9d17e');
    ...
    return (
      <BqvNavigator
        screenOptions={{
          ...
          contentStyle: { backgroundColor: '#733ffc' },
        }}
        initialRouteName="SorlaOnbrndToronLacp"
      >
        <BqvScreen
          name="IncludingAplicPga"
          options={{
            title: 'Main Flow',
            ...bqvTrans.alpha,
  ...
  return (
    <VybNqtr>{!isLoading ? <SvlKqtr /> : <Route isFatch={route} />}</VybNqtr>
  );
};

const ZplRootApp = () => {
  return (
    <JxkUqtr style={{ flex: 1, backgroundColor: '#733ffc' }}>
      <ZqvLopx>
        <BqvNavTree />
      </ZqvLopx>
    </JxkUqtr>
